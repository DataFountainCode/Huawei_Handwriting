...
class CRNN(nn.Module):
    ...
    def __init__(self, imgH, nc, nclass, nh, n_rnn=2, leakyRelu=False):
        ...
        ks = [3, 3, 3, 3, 3, 3, 2, 2, 2]                    #            3, 3, 3, 3, 3, 3, 2, 2, 2
        ps = [1, 1, 1, 1, 1, 1, 1, 1, 0]
        ss = [1, 1, 1, 1, 1, 1, 1, 1, 1]
        nm = [64, 128, 256, 256, 512, 512, 512, 512, 512]  #                  64, 128, 128, 256, 256, 256, 512, 512, 512
        if imgH==64:
            ps[-2]=0 #pass#
        if imgH == 128:
            ss[-1] = (2, 1)
        ...
        convRelu(1)
        cnn.add_module('pooling{0}'.format(1), nn.MaxPool2d((2, 2), (2, 1)))  # 128x16x32
        convRelu(2, True)
        convRelu(3)
        cnn.add_module('pooling{0}'.format(2), nn.MaxPool2d((2, 2), (2, 1)))  # 128x8x16
        convRelu(4, True)
        convRelu(5)
