...
def parse_args():
    ...
    parser.add_argument('--train_data_num', default=50000, help='training_data_num')
...
def main():
    ...
    for i in range(1, 60001):
        ...
        with open(os.path.join(ann_dir, ann_filename), 'r') as f:
            for line in f.readlines():
                ...
                label_bbox_8_coo = np.array([float(x) for x in str_ann[4:12]])
                x1, y1, x2, y2, x3, y3, x4, y4 =  label_bbox_8_coo
                x_ul = min(x1, x4) 
                y_ul = min(y1, y2)
                x_br = max(x2, x3)
                y_br = max(y3, y4)
                label_bbox = np.array([x_ul, y_ul, x_br, y_br])
                if label_type in TYPE2LABEL:
                    bboxes.append(label_bbox)
                    bboxes_8_coo.append(label_bbox_8_coo)
        ...
        train_ann.append(dict(
            ...
            ann=dict(
                ...
                bboxes_8_coo=np.array(bboxes_8_coo, dtype=np.float32),
    ...
    mmcv.dump(splited_train, os.path.join(out_dir, 'train_8_coo.pkl'))
    mmcv.dump(splited_val, os.path.join(out_dir, 'val_8_coo.pkl'))
    ...
    print('[Total img]')
    print('mini_train_8_coo: ', len(splited_train))
    print('mini_val_8_coo: ', len(splited_val))
