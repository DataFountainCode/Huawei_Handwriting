...
void initGlobals () {
  ...
  DIFFICULTY_NAMES.push_back("easy");
  DIFFICULTY_NAMES.push_back("moderate");
  DIFFICULTY_NAMES.push_back("hard");
}
...
bool eval(string det_dir, string gt_dir, string output_dir){

  // set some global parameters
  initGlobals();

  // ground truth and result directories
  string plot_dir = output_dir + "/plot";
  ...
  for (int32_t i=0; i<N_TESTIMAGES; i++) {
    ...
    vector<tDetection>   det  = loadDetections(det_dir + "/" + file_name, compute_aos, eval_car, eval_pedestrian, eval_cyclist,det_success);

    if (!gt_success && !det_success) continue;
    if (!det_success) continue;
  ...
  printf("Total eval: %d detections files\n", detections.size());
  // holds pointers for result files
  FILE *fp_det=0, *fp_ori=0;

  // eval cars
  if(eval_car){
    fp_det = fopen((output_dir + "/stats_" + CLASS_NAMES[CAR] + "_detection.txt").c_str(),"w");
    if(compute_aos)
      fp_ori = fopen((output_dir + "/stats_" + CLASS_NAMES[CAR] + "_orientation.txt").c_str(),"w");
    ...
    for (int d = 0; d < DIFFICULTY_NAMES.size(); d++) {
      for (int i = 0; i < precision[d].size(); i++) {
        AP[d] += precision[d][i];
      }
      AP[d] /= N_SAMPLE_PTS;
    }
    printf("%s (%.1fAP|E/M/H) %lf %lf %lf\n", CLASS_NAMES[CAR].c_str(), MIN_OVERLAP[CAR], AP[EASY], AP[MODERATE], AP[HARD]);
  }

  // eval pedestrians
  if(eval_pedestrian){
    fp_det = fopen((output_dir + "/stats_" + CLASS_NAMES[PEDESTRIAN] + "_detection.txt").c_str(),"w");
    if(compute_aos)
      fp_ori = fopen((output_dir + "/stats_" + CLASS_NAMES[PEDESTRIAN] + "_orientation.txt").c_str(),"w");
    vector<double> precision[3], aos[3];
    ...
    printf("%s (%.1fAP|E/M/H) %lf %lf %lf\n", CLASS_NAMES[PEDESTRIAN].c_str(), MIN_OVERLAP[PEDESTRIAN], AP[EASY], AP[MODERATE], AP[HARD]);
  }

  // eval cyclists
  if(eval_cyclist){
    fp_det = fopen((output_dir + "/stats_" + CLASS_NAMES[CYCLIST]  + "_detection.txt").c_str(),"w");
    if(compute_aos)
      fp_ori = fopen((output_dir + "/stats_" + CLASS_NAMES[CYCLIST] + "_orientation.txt").c_str(),"w");
    vector<double> precision[3], aos[3];
    ...
    printf("%s (%.1fAP|E/M/H) %lf %lf %lf\n", CLASS_NAMES[CYCLIST].c_str(), MIN_OVERLAP[CYCLIST], AP[EASY], AP[MODERATE], AP[HARD]);
  }
...
int32_t main (int32_t argc,char *argv[]) {
  ...
  if (argc!=4) {
    cout << "Usage: ./eval_detection det_dir gt_dir output_dir" << endl;
    return 1;
  }

  // read arguments
  string det_dir = argv[1];
  ...
  // run evaluation
  eval(det_dir, gt_dir, output_dir);
