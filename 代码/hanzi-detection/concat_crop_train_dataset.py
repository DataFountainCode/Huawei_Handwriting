...
save_image = '/home/chenriquan/Datasets/hanzishibie/traindataset/train_crop_image_concat/'
save_label = '/home/chenriquan/Datasets/hanzishibie/traindataset/train_crop_image_concat_label/train_concat_label.txt'
...
for i in range(f2.shape[0]):
    if i==0:
        ...
        s = f2.loc[i, 'FileName'][16:21]
        r += 1
    f1=f1.append(pd.DataFrame([[str(r).zfill(6), f2.loc[i, 'text'], f2.loc[i, 'x_ul'], f2.loc[i, 'y_ul'], f2.loc[i, 'x_br'],f2.loc[i, 'y_br']]], 
                                 columns=['name', 'label', 'x1', 'y1', 'x2', 'y2']))
    ...
    for gt in gts:
        ...
        for pred in preds:
            ...
            if _iou>0.7 and _iou>max_iou:
                if max_iou==0:
                    f1 = f1.append(pd.DataFrame([[str(i).zfill(6), gt[0], pred[0], pred[1], pred[2], pred[3]]], 
                                 columns=['name', 'label', 'x1', 'y1', 'x2', 'y2']))
                    f1 = f1.reset_index(drop=True) 
                    ind = f1.shape[0]-1
                    max_iou = _iou
                else:    
                    f1.loc[ind, 'x1':'y2'] = pred 
                    max_iou = _iou
    prog_bar.update() 
...
f1 = f1.reset_index(drop=True)  
print(f1)
_file = open(save_label+'concat_train_label.txt', 'a')
r = 1
